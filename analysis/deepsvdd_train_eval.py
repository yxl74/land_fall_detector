#!/usr/bin/env python3
"""
Train and evaluate a benign-only Deep SVDD anomaly detector.
...
def main() -> int:
    ...
    parser.add_argument("--output-json", default="outputs/deepsvdd_metrics.json")
    parser.add_argument("--output-model", default="outputs/deepsvdd_model.keras")
    ...
    # Compute center c from initial network outputs
    ...
    thr = float(np.percentile(scores[val_benign_idx], args.threshold_percentile))

    y_true = np.where(labels == "benign", 0, 1).astype(np.int64)
    val_metrics = metrics_at_threshold(y_true[val_idx], scores[val_idx], thr)
    test_metrics = metrics_at_threshold(y_true[test_idx], scores[test_idx], thr)
    test_metrics["roc_auc"] = roc_auc(y_true[test_idx], scores[test_idx])
